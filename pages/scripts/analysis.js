  ...
  let myChart;  // Declare variable to hold chart instance
  let graphType = "bar"; // Default graph type

  // Subjects to compare
  const subjects = ["English","LifeSkills","ProblemSolving","Tech","Total"]; // Add more subjects as needed
  const subjectColors = ['rgba(75, 192, 192, 0.2)', 'rgba(153, 102, 255, 0.2)',"red","Brown","green"]; // Colors for background
  const subjectBorderColors = ['rgba(75, 192, 192, 1)', 'rgba(153, 102, 255, 1)',"red","Brown","green"]; // Colors for border
  ...
  async function getMarksData(path) {
    ...
    data.sort((a, b) => a[subjects[0]] - b[subjects[0]]); // Sort by Total marks
  ...
  async function createChart() {
    // Fetch data from the specified path in Realtime Database
    const marksData = await getMarksData('/studentMarks/ClassA/months/February/result');
