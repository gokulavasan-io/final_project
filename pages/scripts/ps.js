...
function getAllProblemSolvingNames() {
    const dbRef = ref(database);
    const dataPath = "studentMarks/ProblemSolving";
    ...
    get(child(dbRef, dataPath)).then((snapshot) => {
        if (snapshot.exists()) {
            ...
            console.log("No data available in ProblemSolvingMarks.");
            const container = document.querySelector('.marks-container');
            const div = document.createElement('div');
            div.classList.add('marks-detail');
            const pElement = document.createElement('p');
...
function deleteSelectedDatasets() {
    ...
    selectedCheckboxes.forEach(checkbox => {
        ...
        const datasetRef = ref(database, `studentMarks/ProblemSolving/${datasetName}`);
